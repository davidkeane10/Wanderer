...
// Defined outside SearchScreen so React sees the same type reference each render.
// Inner function components cause React to unmount/remount the subtree every
// render, which breaks useNativeDriver animations on device.
...
function Header({ step, onBack, aiOnline, cityName, onAddSpot, onLocationPress }: HeaderProps) {
  return (
    <View style={styles.headerRow}>
      {step > 1 ? (
        ...
        <TouchableOpacity style={styles.addSpotBtn} onPress={onAddSpot} activeOpacity={0.8}>
...
function LocationGate({ onPress }: { onPress: () => void }) {
  return (
    <View style={styles.gateContainer}>
      ...
      <TouchableOpacity style={styles.gateGpsBtn} onPress={onPress} activeOpacity={0.85}>
        <Ionicons name="locate" size={20} color="#fff" />
        <Text style={styles.gateGpsBtnText}>Use my location</Text>
      </TouchableOpacity>
      <TouchableOpacity style={styles.gateCityBtn} onPress={onPress} activeOpacity={0.85}>
...
interface CategoryCardProps {
  cfg: CategoryConfig;
  onPress: (cfg: CategoryConfig) => void;
}

function CategoryCard({ cfg, onPress }: CategoryCardProps) {
  return (
    <TouchableOpacity
...
interface StepCategoryProps {
  onSelectCategory: (cfg: CategoryConfig) => void;
}

function StepCategory({ onSelectCategory }: StepCategoryProps) {
  return (
    <View style={[styles.stepContent, styles.stepCategoryFill]}>
      <Text style={styles.stepTitle}>What are you{"\n"}looking to do?</Text>
      <Text style={styles.stepSubtitle}>Choose a category to get started</Text>
      <View style={styles.categoryGrid}>
        <View style={styles.categoryRow}>
          <CategoryCard cfg={CATEGORIES[0]} onPress={onSelectCategory} />
          <CategoryCard cfg={CATEGORIES[1]} onPress={onSelectCategory} />
        </View>
        <View style={styles.categoryRow}>
          <CategoryCard cfg={CATEGORIES[2]} onPress={onSelectCategory} />
          <CategoryCard cfg={CATEGORIES[3]} onPress={onSelectCategory} />
        </View>
      </View>
    </View>
  );
...
interface StepDistanceProps {
  ...
  onSetDistanceIndex: (i: number) => void;
  onNext: () => void;
}
        ...
        {KM_STEPS.map((km, idx) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => onSetDistanceIndex(idx)}
            >
...
export function SearchScreen({ onSearch }: SearchScreenProps) {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      if (!online && !cancelled) setTimeout(check, 10_000);
    }
    ...
    return (
      <View style={[styles.container, { paddingTop: insets.top }]}>
        <Header
          ...
          aiOnline={aiOnline}
          cityName={location.cityName}
          onAddSpot={() => router.push("/submit-spot" as any)}
          onLocationPress={() => router.push("/location-picker")}
        />
        <View style={styles.gateFull}>
          <LocationGate onPress={() => router.push("/location-picker")} />
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        >
          {/* Single Animated.View persists across steps — prevents native node remount */}
          <Animated.View style={[step === 1 && styles.flex, { opacity }]}>
            {step === 1 && (
            ...
            {step === 2 && selectedCategory && (
              <StepDistance
                ...
                useMetric={useMetric}
                onSetMetric={setUseMetric}
                distanceDisplay={distanceDisplay}
                distanceIndex={distanceIndex}
                onSetDistanceIndex={setDistanceIndex}
...
const styles = StyleSheet.create({
  ...
  // Category grid (2x2) — fills remaining height via flex chain
  // Horizontal padding comes from the ScrollView contentContainerStyle (styles.scroll)
  stepCategoryFill: {
    flex: 1,
    paddingBottom: 12,
  },
